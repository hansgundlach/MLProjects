...
def optimise_input(
    ...
    **kwargs
    ...
    if run_random > 0:
        ...
        target_output = tokenizer.decode(
            random_ix
        )  # Converts token index to string representation
        wandb.config.update({"target_output": target_output}, allow_val_change=True)

    print(
        'Optimising input of length {} to maximise output logits for "{}"'.format(
            input_len, target_output
        )
    )
    done = None
    ...
    metrics_table = wandb.Table(
        columns=["Input", "Output", "Loss", "Perplexity", "Distance", "Probs"]
    )

    if output_len == None or output_len < output_ix.shape[0]:
        output_len = output_ix.shape[0]
    else:
        possible_target_positions = torch.stack(
            [
                torch.arange(0, output_ix.shape[0]) + i
                for i in range(output_len - output_ix.shape[0] + 1)
            ]
        )

    if rand_input == True:
        start_input = word_embeddings[
            torch.randperm(word_embeddings.shape[0])[: input_len * batch_size]
        ].reshape(batch_size, input_len, -1)
    elif local_input == True:
        local_embs = closest_tokens(
            word_embeddings[output_ix].mean(dim=0),
            word_embeddings,
            tokenizer,
            n=batch_size,
        )[-1].unsqueeze(1)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimiser, "min", patience=20, cooldown=20, factor=0.5
    )
    ...
    for e in range(epochs):
        ...
        if output_len > output_ix.shape[0]:
            target_logits = logits[:, possible_target_positions, output_ix].max(dim=1)[
                0
            ]
        ...
        token_dist, closest_ix = torch.stack(token_dist).squeeze(-1), torch.stack(
            closest_ix
        ).squeeze(-1)
        ...
        for b in range(batch_size):
            if target_output in tokenizer.decode(model_outs[b][input_len:]):
                if tokenizer.decode(model_outs[b]) not in optimised_inputs:
                    optimised_tokens += [
                        tokenizer.decode(t) for t in model_outs[b][:input_len]
                    ]
                    ...
                    wandb.log(
                        {
                            "token_freqs": wandb.plot.bar(
                                table, "Token", "Count", title="Token Freqs"
                            )
                        }
                    )
                    ...
                    metrics_table.add_data(
                        *[
                            tokenizer.decode(model_outs[b][:input_len]),
                            tokenizer.decode(model_outs[b][input_len:]),
                        ]
                        + torch.stack(
                            [
                                loss.squeeze(-1)[b].mean(),
                                perp[b],
                                token_dist.mean(dim=1)[b],
                            ],
                            dim=-1,
                        ).tolist()
                        + [target_probs[b].tolist()]
                    )
                    wandb.log(
                        {
                            "Optimised Inputs": wandb.Html(
                                "".join(
                                    [
                                        "<p>{}.{}</p>".format(i, repr(s))
                                        for i, s in enumerate(optimised_inputs)
                                    ]
                                )
                            )
                        }
                ...
                if no_reinit == False:
                    if rand_input == True or local_input == True:
                        input.data[b] = (
                            word_embeddings[
                                torch.randperm(word_embeddings.shape[0])[:input_len]
                            ]
                            .reshape(1, input_len, -1)
                            .to(device)
                        )
                    else:
                        rand_centroids = centroids[
                            np.random.randint(0, batch_size, size=input_len)
                        ].unsqueeze(0)
        ...
        if ((e + 1) % w_freq == 0) or done and return_early:
            ...
            print(
                "{}/{} Output Loss: {} Emb Dist Loss: {} Perp Loss: {} LR: {}".format(
                    e + 1,
                    epochs,
                    batch_loss,
                    mean_token_dist,
            ...
            if verbose == 3:
                print(
                    "Target Probs: {}\nTarget Logits: {}\nInput Dists: {}\nInput Perplexity: {}\n".format(
                        target_probs.detach().cpu().numpy(),
                        target_logits.detach().cpu().numpy(),
                        token_dist.detach().cpu().numpy(),
                        perp.detach().reshape(-1).cpu().numpy(),
                    )
                )
                ...
                if verbose > 0:
                    if verbose == 2:
                        print(
                            b,
                            repr(
                                " Raw embeddings: {}".format(
                                    "".join([closest_tokens(e)[0][0] for e in emb[b]])
                                )
                            ),
                        )
                    ...
                    print(
                        b,
                        repr(
                            " Closest embeddings: {}".format(
                                tokenizer.decode(model_outs[b]), "\n"
                            )
                        ),
                    )
            ...
            wandb.log(
                {
                    "Closest Embeddings": wandb.Html(
                        "".join(
                            [
                                "<p>{}.{}</p>".format(i, repr(ce))
                                for i, ce in enumerate(closest_embeddings)
                            ]
                        )
                    ),
                    "Total Loss": total_loss,
                    "Mean Token Distance": mean_token_dist,
                    "Mean Loss": batch_loss,
                    "Mean Perplexity Loss": perp_loss,
                    "Epoch": e,
                    "LR": optimiser.param_groups[0]["lr"],
                    "Num Inputs Found": len(optimised_inputs),
...
if __name__ == "__main__":
    ...
    args.model, args.word_embeddings, args.tokenizer = load_all(
    ...
    if args.run_test_set > -1:
        for to in test_sets[args.run_test_set]:
            ...
            run = wandb.init(
                config=args, project="backwards", entity=args.wandb_user, reinit=True
            )
